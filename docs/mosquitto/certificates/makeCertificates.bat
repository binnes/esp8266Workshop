...
SET CLIENT_ID_1=dev01
SET CLIENT_ID_2=dev02
...
%OPENSSL_BIN% rsa -in mqttServer_key.pem -passin pass:password123 -out mqttServer_key_nopass.pem
...
%OPENSSL_BIN% req -new -sha256 -subj "/C=%COUNTRY_CODE%/ST=%COUNTY_STATE%/L=%TOWN%/O=MOSQUITTO/OU=MOSQUITTO Corporate/CN=%CLIENT_ID_1%" -passin pass:password123 -key SecuredDev01_key.pem -out SecuredDev01_crt.csr
...
%OPENSSL_BIN% genrsa -aes256 -passout pass:password123 -out SecuredDev02_key.pem 2048
%OPENSSL_BIN% req -new -sha256 -subj "/C=%COUNTRY_CODE%/ST=%COUNTY_STATE%/L=%TOWN%/O=MOSQUITTO/OU=MOSQUITTO Corporate/CN=%CLIENT_ID_2%" -passin pass:password123 -key SecuredDev02_key.pem -out SecuredDev02_crt.csr
%OPENSSL_BIN% x509 -days 3650 -in SecuredDev02_crt.csr -out SecuredDev02_crt.pem -req -sha256 -CA rootCA_certificate.pem -passin pass:password123 -CAkey rootCA_key.pem -set_serial 131
%OPENSSL_BIN% rsa -outform der -in SecuredDev02_key.pem -passin pass:password123 -out SecuredDev02_key.key
%OPENSSL_BIN% rsa -in SecuredDev02_key.pem -passin pass:password123 -out SecuredDev02_key_nopass.pem
%OPENSSL_BIN% x509 -outform der -in SecuredDev02_crt.pem -out SecuredDev02_crt.der
